...
class TestRawLibnetplan(TestBase):
    ...
    def test_get_id_from_filename_rootdir(self):
        out = lib.netplan_get_id_from_nm_filename(
            "/some/rootdir/run/NetworkManager/system-connections/netplan-some-id.nmconnection".encode(),
            None,
        )
        self.assertEqual(out, b"some-id")
    ...
    def test_get_id_from_filename_wifi(self):
        out = lib.netplan_get_id_from_nm_filename(
            "/run/NetworkManager/system-connections/netplan-some-id-SOME-SSID.nmconnection".encode(),
            "SOME-SSID".encode(),
        )
        self.assertEqual(out, b"some-id")
    ...
    def test_get_id_from_filename_wifi_invalid_suffix(self):
        out = lib.netplan_get_id_from_nm_filename(
            "/run/NetworkManager/system-connections/netplan-some-id-SOME-SSID".encode(),
            "SOME-SSID".encode(),
        )
    ...
    def test_get_id_from_filename_invalid_prefix(self):
        out = lib.netplan_get_id_from_nm_filename(
            "INVALID/netplan-some-id.nmconnection".encode(), None
        )
        self.assertEqual(out, None)
  ...
  ethernets:
    some-netplan-id:
      dhcp4: true"""
        ...
        with capture_stderr() as outf:
            self.assertFalse(
                lib.netplan_delete_connection("unknown-id".encode(), self.workdir.name.encode())
            )
            self.assertTrue(os.path.isfile(orig))
            with open(outf.name, "r") as f:
                self.assertIn(
                    "netplan_delete_connection: Cannot delete unknown-id, does not exist.",
                    f.read().strip(),
                )
  ...
  ethernets:
    ...
    def test_delete_connection_invalid(self):
        ...
        with capture_stderr() as outf:
            self.assertFalse(
                lib.netplan_delete_connection(
                    "some-netplan-id".encode(), self.workdir.name.encode()
                )
            )
            with open(outf.name, "r") as f:
  ...
  bridges:
    br0:
      dhcp4: false""",
        ...
        self.assertSetEqual(
            set(["eth0", "br0"]), set(d.id for d in libnetplan._NetdefIterator(state, None))
        )
  ...
  bridges:
    br0:
      dhcp4: false""",
        ...
        self.assertSetEqual(
            set(["eth0", "eth1"]), set(d.id for d in libnetplan._NetdefIterator(state, "ethernets"))
        )
