...
class TestOVS(unittest.TestCase):
    ...
    def test_clear_settings_tag(self, mock):
        ...
        mock.assert_called_with(
            [OVS, "remove", "Bridge", "ovs0", "external-ids", "netplan/external-ids/key"]
        )
...
Bootstrap: false"""
        ovs.clear_setting(
            "Open_vSwitch",
            ".",
            "netplan/global/set-ssl",
            "/private/key.pem,/another/cert.pem,/some/ca-cert.pem",
        )
        ...
        mock.assert_has_calls(
            [
                call([OVS, "del-ssl"]),
                call(
                    [OVS, "remove", "Open_vSwitch", ".", "external-ids", "netplan/global/set-ssl"]
                ),
            ]
        )
...
Bootstrap: false"""
        ovs.clear_setting(
            ...
            ".",
            "netplan/global/set-ssl",
            "/some/key.pem,/other/cert.pem,/some/cert.pem",
        )
        mock_out.assert_called_once_with([OVS, "get-ssl"], text=True)
        mock.assert_has_calls(
    ...
    def test_clear_global(self, mock, mock_out):
        ...
        ovs.clear_setting(
            ...
            "tcp:127.0.0.1:1337,unix:/some/socket",
        )
