...
class TestSRIOV(unittest.TestCase):
    ...
    def _prepare_sysfs_dir_structure(
        self,
        pf=("enp2", "0000:00:1f.0"),
        vfs=[("enp2s16f1", "0000:00:1f.6")],
        pf_driver="fake_driver",
    ):
        """
        ...
        os.symlink(
            os.path.join("../../../devices/pci0000:00", pf_pci_addr),
        ...
        for vf_iface, vf_pci_addr in vfs:
            ...
            # symlink it to /sys/bus/pci/devices
            os.symlink(
                os.path.join("../../../devices/pci0000:00", vf_pci_addr),
            ...
            with open(os.path.join(vf_dev_path, "device"), "w") as f:
                f.write("0xb33f\n")
            os.symlink(
                os.path.join("../../devices/pci0000:00", vf_pci_addr, "net", vf_iface),
                os.path.join(sysfs, "class/net", vf_iface),
            )
            os.symlink(
                os.path.join("../../..", vf_pci_addr),
        ...
        with open(os.path.join(pf_dev_path, "sriov_numvfs"), "w") as f:
            f.write(str(len(vfs)) + "\n")
        os.symlink(
            os.path.join("../../../bus/pci/drivers", pf_driver), os.path.join(pf_dev_path, "driver")
        )
        os.symlink(
            os.path.join("../../devices/pci0000:00", pf_pci_addr, "net", pf_iface),
            os.path.join(sysfs, "class/net", pf_iface),
        )
        os.symlink(
            os.path.join("../../..", pf_pci_addr),
            os.path.join(sysfs, "class/net", pf_iface, "device"),
        )
        # the PF additionally has device links to all the VFs defined for it
        for i in range(len(vfs)):
            os.symlink(
                os.path.join("../../..", vfs[i][1]), os.path.join(pf_dev_path, "virtfn" + str(i))
            )
...
""",
        ...
        sriov.get_vf_count_and_functions(
            interfaces, self.configmanager.np_state, vf_counts, vfs, pfs
        )
        # check if the right vf counts have been recorded in vf_counts
        self.assertDictEqual(vf_counts, {"enp1": 2, "enp2": 2, "enp3": 1, "enp5": 1, "enp8": 7})
        ...
        self.assertDictEqual(
            pfs, {"enp1": "enp1", "enp2": "enp2", "enp3": "enp3", "enpx": "enp5", "enp8": "enp8"}
        )
...
""",
        ...
        self.assertIn("matched more than one interface for a PF device: enpx", str(e.exception))
...
""",
        ...
        self.assertIn("more VFs allocated than the explicit size declared: 3 > 2", str(e.exception))
    ...
    def test_set_numvfs_for_pf(self):
        ...
        self.assertListEqual(
            sriov_open.open.call_args_list,
            [
    ...
    def test_set_numvfs_for_pf_over_max(self):
        ...
        with patch("builtins.open", sriov_open.open):
            ...
            self.assertIn("cannot allocate more VFs for PF enp1 than supported", str(e.exception))
    ...
    def test_set_numvfs_for_pf_over_theoretical_max(self):
        ...
        with patch("builtins.open", sriov_open.open):
            ...
            self.assertIn(
                "cannot allocate more VFs for PF enp1 than the SR-IOV maximum", str(e.exception)
            )
    ...
    def test_set_numvfs_for_pf_write_failed(self):
        ...
        with patch("builtins.open", sriov_open.open):
            ...
            self.assertIn("failed setting sriov_numvfs to 2 for enp1", str(e.exception))
    ...
    def test_perform_hardware_specific_quirks_failed(self):
        ...
        with patch("builtins.open", sriov_open.open):
            for case in cases:
                ...
                self.assertIn("could not determine vendor and device ID of enp1", str(e.exception))
    ...
    def test_apply_vlan_filter_for_vf(self, check_call):
        ...
        self.assertListEqual(
            ...
            ["ip", "link", "set", "dev", "enp2", "vf", "0", "vlan", "10"],
        )
    ...
    def test_apply_vlan_filter_for_vf_failed_no_index(self, check_call):
        ...
        with self.assertRaises(RuntimeError) as e:
            sriov.apply_vlan_filter_for_vf(
                "enp2", "enp2s16f1", "vlan10", 10, prefix=self.workdir.name
            )
    ...
    def test_apply_vlan_filter_for_vf_failed_ip_link_set(self, check_call):
        ...
        self.assertIn("failed setting SR-IOV VLAN filter for vlan vlan10", str(e.exception))
    ...
    @patch("netplan.cli.utils.get_interface_driver_name")
    @patch("netplan.cli.utils.get_interface_macaddress")
    def test_apply_sriov_config(
        self, gim, gidn, apply_vlan, quirks, set_numvfs, get_counts, netifs
    ):
...
""",
        ...
        self.assertListEqual(set_numvfs.call_args_list, [call("enp1", 2), call("enp2", 1)])
    ...
    @patch("netplan.cli.utils.get_interface_driver_name")
    @patch("netplan.cli.utils.get_interface_macaddress")
    def test_apply_sriov_config_invalid_vlan(
        self, gim, gidn, apply_vlan, quirks, set_numvfs, get_counts, netifs
    ):
        # set up the environment
        with open(os.path.join(self.workdir.name, "etc/netplan/test.yaml"), "w") as fd:
...
""",
        ...
        with self.assertLogs() as logs:
            sriov.apply_sriov_config(self.configmanager, rootdir=self.workdir.name)
            self.assertIn(
                "SR-IOV vlan defined for vf1.15 but link enp1 is "
    ...
    @patch("netplan.cli.utils.get_interface_driver_name")
    @patch("netplan.cli.utils.get_interface_macaddress")
    def test_apply_sriov_config_too_many_vlans(
        self, gim, gidn, apply_vlan, quirks, set_numvfs, get_counts, netifs
    ):
        # set up the environment
        with open(os.path.join(self.workdir.name, "etc/netplan/test.yaml"), "w") as fd:
...
""",
    ...
    @patch("netplan.cli.utils.get_interface_driver_name")
    @patch("netplan.cli.utils.get_interface_macaddress")
    def test_apply_sriov_config_many_match(
        self, gim, gidn, apply_vlan, quirks, set_numvfs, get_counts, netifs
    ):
        # set up the environment
        with open(os.path.join(self.workdir.name, "etc/netplan/test.yaml"), "w") as fd:
...
""",
        ...
        self.assertIn(
            "matched more than one interface for a VF device: customvf1", str(e.exception)
        )
...
"""
    ...
    def test_unit_class_PCIDevice(self):
        ...
        with patch(
            "netplan.cli.sriov.PCIDevice.sys", new_callable=unittest.mock.PropertyMock
        ) as sys_mock:
            sys_mock.return_value = os.path.join(self.workdir.name, "sys_mock")
            os.makedirs(
                os.path.join(self.workdir.name, "sys_mock/bus/pci/devices/0000:00:1f.6/driver")
            )
            self.assertTrue(pcidev.bound)
            open(
                os.path.join(self.workdir.name, "sys_mock/bus/pci/devices/0000:00:1f.6/physfn"), "a"
            ).close()
    ...
    def test_apply_sriov_config_eswitch_mode(
        self, gpsn, pcidevice_sys, pcidevice_bound, scc, quirks, set_numvfs, get_counts, netifs
    ):
        handle = mock_open()
        ...
        self._prepare_sysfs_dir_structure(
            pf=("enp1", "0000:03:00.0"),
            vfs=[("enp1s16f1", "0000:03:00.2"), ("enp1s16f2", "0000:03:00.3")],
            pf_driver="mlx5_core",
        )
        self._prepare_sysfs_dir_structure(
            pf=("enp2", "0000:03:00.1"),
            vfs=[
                ("enp2s14f1", "0000:03:08.2"),
        ...
        pcidevice_bound.side_effect = [
            True,
            True,  # 2x unbind (enp1 VFs)
            True,
            True,
            True,
            True,  # 4x unbind (enpx/enp2 VFs)
            False,
            False,
            False,
            False,
        ]  # 4x re-bind (enpx/enp2 VFs)
...
""",
        ...
        scc.assert_has_calls(
            [
                call(
                    ["/sbin/devlink", "dev", "eswitch", "set", "pci/0000:03:00.0", "mode", "legacy"]
                ),
                call(
                    [
                        "/sbin/devlink",
    ...
    def test_cli_rebind(self, gpsn, sys_mock, bound_mock):
        self._prepare_sysfs_dir_structure(
            pf=("enp3s0f0", "0000:03:00.0"),
            vfs=[("enp3s0f0v0", "0000:03:00.2"), ("enp3s0f0v1", "0000:03:00.3")],
        ...
        gpsn.side_effect = (
            lambda iface: enp3s0f0_pci_addr if iface == "enp3s0f0" else not_a_pf_pci_addr
        )
        ...
        with patch("builtins.open", mock_open(read_data="")) as mock_file:
            ...
            self.assertEqual(
                mock_file.return_value.write.call_args_list,
                [call("0000:03:00.2"), call("0000:03:00.3")],
            )
  ...
  ethernets:
    ...
    sriov_vf:
      link: engreen"""
        ...
        self.assert_sriov({})
        self.assertIn(
            "engreen: Cannot rebind SR-IOV virtual functions, unknown interface name.", out
        )
