    ...
    def test_set_mapping(self):
        ...
        with open(self.path, "r") as f:
            ...
            self.assertSequenceEqual(
                ["1.2.3.4/24"], out["network"]["ethernets"]["eth0"]["addresses"]
            )
...
"""
  ...
  tunnels:
    tun0:
      ...
      remote: 5.6.7.8"""
        ...
        self.assertIn(
            "tun0: 'input-key' is not required for this tunnel type", str(context.exception)
        )
  ...
  ethernets:
    ens3: {dhcp4: "yes"}"""
        ...
        self.assertTrue(os.path.isfile(self.path))
        with open(self.path, "r") as f:
            self.assertIs(True, yaml.safe_load(f)["network"]["ethernets"]["ens3"]["dhcp4"])
  ...
  bridges:
    br0:
      ...
      parameters:
        ...
        self._set(
            [
                "network.bridges.br0.parameters={path-cost: {eno1: null}, port-priority: {eno2: null}}"
            ]
        )
...
"""
  ...
  ethernets:
    ...
    def test_set_invalid_input(self):
        ...
        self.assertIn("unknown key 'dhcp4:false'", str(context.exception))
    ...
    def test_set_override_multiple_existing_files(self):
        ...
        self._set(
            [
                (
                    r'network={"renderer": "NetworkManager", "version":2,'
                    r'"ethernets":{'
                    r'"eth1":{"dhcp4":false},'
    ...
    ens3:
      ...
      addresses: [1.2.3.4/24, 5.6.7.8/24]"""
        ...
        self.assertDictEqual(
            {"ens3": {"addresses": ["1.2.3.4/24", "5.6.7.8/24"], "dhcp4": True}}, out
        )
...
"""
        ...
        self.assertListEqual(
            ["match", "dhcp4", "set-name", "mtu", "virtual-function-count"], list(out)
        )
