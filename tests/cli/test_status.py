...
DNS_IP6 = bytearray(
    [0xFD, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xCE, 0xCE, 0x1E, 0xFF, 0xFE, 0x3D, 0xC7, 0x37]
)
DNS_ADDRESSES = [
    (5, 2, DNS_IP4),
    (5, 10, DNS_IP6),
...
class TestStatus(unittest.TestCase):
    ...
    def test_query_iproute2(self, mock):
        ...
        self.assertListEqual(
            [itf.get("ifname") for itf in res], ["lo", "enp0s31f6", "wlan0", "wg0", "wwan0", "tun0"]
        )
    ...
    def test_query_networkd(self, mock):
        ...
        self.assertEqual(len(res), 6)
        self.assertListEqual(
            [itf.get("Name") for itf in res], ["lo", "enp0s31f6", "wlan0", "wg0", "wwan0", "tun0"]
        )
    ...
    def test_query_nm(self, mock):
        ...
        mock.assert_called_with(
            ["nmcli", "-t", "-f", "DEVICE,NAME,UUID,FILENAME,TYPE,AUTOCONNECT", "con", "show"],
            text=True,
        )
    ...
    def test_query_resolved(self, mock_ipc, mock_iface):
        ...
        self.assertListEqual([addr[0] for addr in addresses], [5, 5, 2, 2])  # interface index
        self.assertEqual(len(search), 2)
        self.assertListEqual([s[1] for s in search], ["search.domain", "search.domain"])
...
# This is /run/systemd/resolve/resolv.conf managed by man:systemd-resolved(8)."
    ...
    def test_pretty_print(self, networkctl_mock, nm_ssid_mock):
        ...
        networkctl_mock.return_value = """Activation Policy: manual
            WiFi access point: {} (b4:fb:e4:75:c6:21)""".format(
            SSID
        )
  ...
  online: false""".strip(),
    ...
    def test_call_cli_no_networkd(
        ...
        with self.assertLogs(level="DEBUG") as cm:
            self._call([])
            self.assertIn(
                "DEBUG:root:systemd-networkd.service is not active. Starting...", cm.output[0]
            )
...
class TestInterface(unittest.TestCase):
    ...
    def test_query_nm_ssid(self, mock):
        ...
        mock.assert_called_with(
            ["nmcli", "--get-values", "802-11-wireless.ssid", "con", "show", "id", con], text=True
        )
    ...
    def test_query_nm_ssid_fail(self, mock):
        ...
        with self.assertLogs() as cm:
            res = itf.query_nm_ssid(con)
            mock.assert_called_with(
                ["nmcli", "--get-values", "802-11-wireless.ssid", "con", "show", "id", con],
                text=True,
            )
            self.assertIsNone(res)
            self.assertIn(
                "WARNING:root:Cannot query NetworkManager SSID for {}:".format(con), cm.output[0]
            )
    ...
    def test_json_nm_wlan0(self, networkctl_mock, nm_ssid_mock):
        ...
        nm_ssid_mock.return_value = SSID
        # networkctl mock output reduced to relevant lines
        networkctl_mock.return_value = "WiFi access point: {} (b4:fb:e4:75:c6:21)".format(SSID)
