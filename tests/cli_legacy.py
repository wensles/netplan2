...
class TestGenerate(unittest.TestCase):
    ...
    def test_no_config(self):
        p = subprocess.Popen(
            exe_cli + ["generate", "--root-dir", self.workdir.name],
  ...
  ethernets:
    enlol: {dhcp4: yes}"""
        ...
        out = subprocess.check_output(
            exe_cli + ["generate", "--root-dir", self.workdir.name], stderr=subprocess.STDOUT
        )
        self.assertEqual(out, b"")
        self.assertEqual(
            os.listdir(os.path.join(self.workdir.name, "run", "systemd", "network")),
  ...
  ethernets:
    enlol: {dhcp4: yes}"""
        ...
        p = subprocess.Popen(
            exe_cli + ["generate", "--root-dir", self.workdir.name, "--mapping", "nonexistent"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
        (out, err) = p.communicate()
  ...
  ethernets:
    enlol: {dhcp4: yes}"""
        ...
        out = subprocess.check_output(
            exe_cli + ["generate", "--root-dir", self.workdir.name, "--mapping", "enlol"]
        )
...
"""
        ...
        out = subprocess.check_output(
            exe_cli + ["generate", "--root-dir", self.workdir.name, "--mapping", "renamediface"]
        )
        self.assertNotEqual(b"", out)
...
class TestIfupdownMigrate(unittest.TestCase):
    ...
    def test_system(self):
        ...
        rc = subprocess.call(
            exe_cli + ["migrate", "--dry-run"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
...
source-directory /etc/network/interfaces.d"""
    ...
    def test_dhcp4(self):
        ...
        self.assertEqual(
            _load_yaml(out),
            {"network": {"version": 2, "ethernets": {"en1": {"dhcp4": True}}}},
            out.decode(),
        )
    ...
    def test_dhcp6(self):
        ...
        self.assertEqual(
            _load_yaml(out),
            {"network": {"version": 2, "ethernets": {"en1": {"dhcp6": True}}}},
            out.decode(),
        )
    ...
    def test_dhcp4_and_6(self):
        out = self.do_test(
            "auto lo\niface lo inet loopback\n\n"
            "auto en1\niface en1 inet dhcp\niface en1 inet6 dhcp"
        )[0]
        self.assertEqual(
            _load_yaml(out),
            {"network": {"version": 2, "ethernets": {"en1": {"dhcp4": True, "dhcp6": True}}}},
            out.decode(),
        )
    ...
    def test_includedir_rel(self):
        out = self.do_test(
            ...
            dropins={
                "interfaces.d/std": "auto en1\niface en1 inet dhcp",
                "interfaces.d/std.bak": "some_bogus dontreadme",
            },
        )[0]
        self.assertEqual(
            _load_yaml(out),
    ...
    def test_includedir_abs(self):
        out = self.do_test(
            ...
            dropins={
                "defs/my/std": "auto en1\niface en1 inet dhcp",
                "defs/my/std.bak": "some_bogus dontreadme",
            },
        )[0]
        self.assertEqual(
            _load_yaml(out),
    ...
    def test_include_rel(self):
        out = self.do_test(
            ...
            dropins={
                "interfaces.d/std.cfg": "auto en1\niface en1 inet dhcp",
                "interfaces.d/std.cfgold": "some_bogus dontreadme",
            },
        )[0]
        self.assertEqual(
            _load_yaml(out),
    ...
    def test_include_abs(self):
        out = self.do_test(
            ...
            dropins={
                "std.cfg": "auto en1\niface en1 inet dhcp",
                "std.cfgold": "some_bogus dontreadme",
            },
        )[0]
        self.assertEqual(
            _load_yaml(out),
    ...
    def test_allow(self):
        ...
        self.assertEqual(
            _load_yaml(out),
            {
                "network": {
                    "version": 2,
                    "ethernets": {"en1": {"dhcp4": True}, "en2": {"dhcp4": True}},
                }
            },
            out.decode(),
        )
    ...
    def test_write_file_haveconfig(self):
        ...
        with open(self.converted_path) as f:
            config = _load_yaml(f)
        self.assertEqual(config, {"network": {"version": 2, "ethernets": {"en1": {"dhcp4": True}}}})
    ...
    def test_static_ipv4_prefix(self):
        ...
        self.assertEqual(
            _load_yaml(out),
            {"network": {"version": 2, "ethernets": {"en1": {"addresses": ["1.2.3.4/8"]}}}},
            out.decode(),
        )

    def test_static_ipv4_netmask(self):
        out = self.do_test(
            "auto en1\niface en1 inet static\naddress 1.2.3.4\nnetmask 255.0.0.0", dry_run=True
        )[0]
        self.assertEqual(
            _load_yaml(out),
            {"network": {"version": 2, "ethernets": {"en1": {"addresses": ["1.2.3.4/8"]}}}},
    ...
    def test_static_ipv6_invalid_addr(self):
        out, err = self.do_test(
            "auto en1\niface en1 inet6 static\n" "address fc00:0123:4567:89ab:cdef::12345/64",
            expect_success=False,
        )
        self.assertEqual(out, b"")
    ...
    def test_static_ipv6_invalid_prefixlen(self):
        out, err = self.do_test(
            "auto en1\niface en1 inet6 static\n" "address fc00:0123:4567:89ab:cdef::1234/129",
            expect_success=False,
        )
        self.assertEqual(out, b"")
        self.assertIn(b'error parsing "fc00:0123:4567:89ab:cdef::1234/129" as an IPv6 network', err)
        ...
        self.assertEqual(
            _load_yaml(out),
            {
                "network": {
                    ...
                    "ethernets": {
                        "en1": {
                            "addresses": ["fc00:123:4567:89ab:cdef::1234/64"],
                            "accept_ra": False,
                        }
                    },
                }
                ...
                "network": {
                    ...
                    "ethernets": {
                        "en1": {
                            ...
                            "gateway4": "1.1.1.1",
                            "gateway6": "fc00:0123:4567:89ab::1",
                        }
                    },
                }
            },
...
iface en1 inet6 static
  address fc00:0123:4567:89ab:cdef::1234/64
  dns-nameservers fc00:0123:4567:89ab:1::1  fc00:0123:4567:89ab:2::1""",
            dry_run=True,
        )[0]
        self.assertEqual(
            _load_yaml(out),
                        ...
                        "en1": {
                            ...
                            "nameservers": {
                                ...
                                "addresses": [
                                    "1.2.1.1",
                                    "1.2.2.1",
                                    "fc00:0123:4567:89ab:1::1",
                                    "fc00:0123:4567:89ab:2::1",
                                ],
                            },
                        }
                    },
            ...
            _load_yaml(out),
            {
                "network": {
                    ...
                    "ethernets": {
                        "en1": {
                            "addresses": ["1.2.3.4/8"],
                            "nameservers": {"search": ["foo", "foo.bar"]},
                        }
                    },
                }
            },
    ...
    def test_static_mtu(self):
        ...
        self.assertEqual(
            _load_yaml(out),
            {
                "network": {
                    "version": 2,
                    "ethernets": {"en1": {"addresses": ["1.2.3.4/8"], "mtu": 1280}},
                }
            },
            out.decode(),
        )
        ...
        self.assertEqual(
            _load_yaml(out),
            {
                "network": {
                    "version": 2,
                    "ethernets": {
                        "en1": {"addresses": ["1.2.3.4/8"], "macaddress": "52:54:00:6b:3c:59"}
                    },
                }
            },
            out.decode(),
    ...
    def test_static_two_different_macs(self):
        out, err = self.do_test(
            "auto en1\niface en1 inet static\naddress 1.2.3.4/8\nhwaddress 52:54:00:6b:3c:59\n"
            "iface en1 inet6 static\naddress 2001::1/64\nhwaddress 52:54:00:6b:3c:58",
            expect_success=False,
        )
        self.assertEqual(b"", out)
        self.assertIn(
            b"tried to set MAC 52:54:00:6b:3c:58, but already have MAC 52:54:00:6b:3c:59", err
        )
...
class TestIp(unittest.TestCase):
    ...
    def test_valid_subcommand(self):
        p = subprocess.Popen(exe_cli + ["ip"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        (out, err) = p.communicate()
        self.assertEqual(out, b"")
...
"""
        ...
        fake_netif_lease_dir = os.path.join(self.workdir.name, "run", "systemd", "netif", "leases")
        ...
        out = subprocess.check_output(
            exe_cli + ["ip", "leases", "--root-dir", self.workdir.name, "lo"]
        )
    ...
    def test_ip_leases_nm(self):
        ...
        new_path = os.pathsep.join([os.path.dirname(mock.path), os.environ.get("PATH", os.defpath)])
...
"""
        ...
        fake_lease_dir = os.path.join(self.workdir.name, "var", "lib", "NetworkManager")
        ...
        out = subprocess.check_output(
            exe_cli + ["ip", "leases", "--root-dir", self.workdir.name, "lo"], env=mock_env
        )
...
"""
        ...
        p = subprocess.Popen(
            exe_cli + ["ip", "leases", "--root-dir", self.workdir.name, "enlol"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
        (out, err) = p.communicate()
