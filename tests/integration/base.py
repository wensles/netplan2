...
test_backends = (
    ...
    if "NETPLAN_TEST_BACKENDS" not in os.environ
    else os.environ["NETPLAN_TEST_BACKENDS"]
)
...
class IntegrationTestsBase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        subprocess.call(
            ["systemctl", "reset-failed", "NetworkManager", "systemd-networkd"],
    ...
    def create_devices(klass):
        ...
        subprocess.check_call(
            ["ip", "link", "add", "name", "eth42", "type", "veth", "peer", "name", "veth42"]
        )
        ...
        subprocess.check_call(
            ["ip", "link", "add", "name", "eth43", "type", "veth", "peer", "name", "veth43"]
        )
        ...
        time.sleep(0.1)
        out = subprocess.check_output(["ip", "-br", "link", "show", "dev", "eth42"], text=True)
        klass.dev_e_client_mac = out.split()[2]
        out = subprocess.check_output(["ip", "-br", "link", "show", "dev", "eth43"], text=True)
    ...
    def shutdown_devices(klass):
        ...
        subprocess.call(["ip", "link", "del", "dev", "mybr"], stderr=subprocess.PIPE)
        subprocess.call(["ip", "link", "del", "dev", "nptestsrv"], stderr=subprocess.PIPE)
    ...
    def assert_iface(self, iface, expected_ip_a=None, unexpected_ip_a=None):
        ...
        out = subprocess.check_output(["ip", "-d", "a", "show", "dev", iface], text=True)
        if expected_ip_a:
    ...
    def generate_and_settle(self, wait_interfaces=None, state_dir=None):
        ...
        ifaces = (
            wait_interfaces
            if wait_interfaces is not None
            else [self.dev_e_client, self.dev_e2_client]
        )
    ...
    def state_dhcp4(self, iface):
        ...
        return self.state(
            iface, "inet 192.168."
        )  # TODO: make this a regex to check for specific DHCP ranges
...
class IntegrationTestsWifi(IntegrationTestsBase):
    ...
    def start_hostapd(self, conf):
        ...
        p = subprocess.Popen(
            ["hostapd", "-e", self.entropy_file, "-f", log, hostapd_conf], stdout=subprocess.PIPE
        )
