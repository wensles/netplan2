    ...
    enmac:
      ...
      dhcp4: yes"""
        ...
        self.assert_iface_up(
            self.dev_e2_client, ["inet 192.168.6.[0-9]+/24", "ether 00:01:02:03:04:05"]
        )
...
"""
                % {"r": self.backend}
            )  # globbing match on "eth42", i.e. self.dev_e_client
...
"""
        ...
        self.assert_iface_up(
            self.dev_e_client,
            ["inet 172.16.42.99/18", "inet6 1234:ffff::42/64", "inet 192.168.5.[0-9]+/24"],
        )  # from DHCP
        self.assert_iface_up(self.dev_e2_client, ["inet 172.16.1.2/24"])
        ...
        if self.backend == "NetworkManager" and nm_uses_dnsmasq:
            ...
            out = subprocess.check_output(
                ["journalctl", "--quiet", "-tdnsmasq", "-ocat", "--since=-30s"], text=True
            )
...
"""
        ...
        self.assert_iface_up(
            self.dev_e_client,
            ["inet 172.16.5.3/20"],
            ["inet 192.168.5", "inet 172.16.42", "inet6 1234"],  # old DHCP  # old static IPv4
        )  # old static IPv6
        self.assert_iface_up(
            self.dev_e2_client,
            [
        ...
        self.assertIn(
            b"default via 192.168.6.1",  # from DHCP
            subprocess.check_output(["ip", "route", "show", "dev", self.dev_e2_client]),
        )
        self.assertNotIn(
            b"default",
