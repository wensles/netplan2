...
class _CommonParserErrors:
    ...
    def test_fail_invalid_public_key(self):
        ...
        config = prepare_wg_config(
            listen=12345,
            privkey="4GgaQCy68nzNsUE5aJ9fuLzHhB65tAlwbmA72MWnOm8=",
            peers=[
                {
    ...
    def test_fail_allowed_ips_noprefix4(self):
        ...
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: address '0.0.0.0' is missing /prefixlength", out
        )
    ...
    def test_fail_allowed_ips_noprefix6(self):
        ...
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: address '2001:fe:ad:de:ad:be:ef:1' is missing /prefixlength",
            out,
        )
    ...
    def test_fail_allowed_ips_bogus(self):
        ...
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: malformed address '302.302.302.302/24', \
must be X.X.X.X/NN or X:X:X:X:X:X:X:X/NN",
            out,
        )
    ...
    def test_fail_missing_private_key(self):
        ...
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: wg0: missing 'key' property (private key) for wireguard",
            out,
        )
    ...
    def test_fail_no_allowed_ips(self):
        ...
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: wg0: 'to' is required to define the allowed IPs.", out
        )
            ...
            self.assert_networkd(
                {
                    "wg0.netdev": ND_WG
                    % (
                        "=4GgaQCy68nzNsUE5aJ9fuLzHhB65tAlwbmA72MWnOm8=",
                        "12345",
                        """FwMark=42
...
PresharedKey=7voRZ/ojfXgfPOlswo3Lpma1RJq7qijIEEUEMShQFV8=""",
                    ...
                    "wg0.network": ND_WITHIPGW
                    % ("wg0", "15.15.15.15/24", "2001:de:ad:be:ef:ca:fe:1/128", "20.20.20.21"),
                }
            )
        elif self.backend == "NetworkManager":
            self.assert_nm(
                {
                    "wg0.nmconnection": NM_WG
                    % (
                        "4GgaQCy68nzNsUE5aJ9fuLzHhB65tAlwbmA72MWnOm8=",
                        "12345",
                        """fwmark=42
...
PresharedKeyFile=/tmp/test_preshared_key""",
            ...
            self.assertIn(
                "wg0: private key needs to be base64 encoded when using the NM backend", err
            )
    ...
    br0:
      interfaces: [vxlan1005]"""
        ...
        if self.backend == "networkd":
            self.assert_networkd(
                {
                    "vxlan1005.netdev": ND_VXLAN % ("vxlan1005", 1005)
                    + """Group=224.0.0.5
Local=10.10.10.1
TOS=292
TTL=128
...
Kind=bridge\n""",
            ...
            self.assertIn(
                "checksums/extensions/flow-lable/do-not-fragment are "
                "not supported by NetworkManager",
                out,
            )
            self.assert_nm(
                {
                    "vxlan1005": """[connection]
...
RouteShortCircuit=true\n""",
                    ...
                    "eth0.network": ND_EMPTY % ("eth0", "ipv6") + """VXLAN=vxlan1005\n""",
                }
            )
...
"""
    ...
    def test_gre_with_keys(self):
        ...
        config = prepare_config_for_mode(
            "NetworkManager", "gre", key={"input": 1111, "output": 5555}
        )
        self.generate(config)
        self.assert_nm(
            {
  ...
  ethernets:
    id0:
      match: {name: 'someIface'}"""
        ...
        with open(
            os.path.join(
                self.workdir.name, "run/NetworkManager/system-connections/netplan-id0.nmconnection"
            )
        ) as f:
...
"""
    ...
    def test_invalid_mode(self):
        ...
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: tunnel mode 'invalid' is not supported", out
        )
    ...
    def test_invalid_mode_for_nm(self):
        ...
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: GRETAP tunnel mode is not supported by NetworkManager",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: malformed address '10.10.1invalid', must be X.X.X.X or X:X:X:X:X:X:X:X",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: address '10.10.10.10/21' should not include /prefixlength",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: missing 'remote' property for tunnel", out
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: 'ttl' property for tunnel must be in range [1...255]",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: 'local' must be a valid IPv4 address for this tunnel type",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: 'remote' must be a valid IPv4 address for this tunnel type",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: 'local' must be a valid IPv6 address for this tunnel type",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: 'remote' must be a valid IPv6 address for this tunnel type",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: invalid type for 'key[s]': must be a scalar or mapping",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: 'input-key' is not required for this tunnel type",
            out,
        )
...
"""
        out = self.generate(config, expect_fail=True)
        self.assertIn(
            "Error in network definition: tun0: 'output-key' is not required for this tunnel type",
            out,
        )
