...
exe_generate = os.environ.get(
    "NETPLAN_GENERATE_PATH",
    os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "generate"
    ),
)
...
"""
NM_MANAGED = (
    'SUBSYSTEM=="net", ACTION=="add|change|move", ENV{ID_NET_NAME}=="%s", ENV{NM_UNMANAGED}="0"\n'
)
NM_UNMANAGED = (
    'SUBSYSTEM=="net", ACTION=="add|change|move", ENV{ID_NET_NAME}=="%s", ENV{NM_UNMANAGED}="1"\n'
)
NM_MANAGED_MAC = (
    'SUBSYSTEM=="net", ACTION=="add|change|move", ATTR{address}=="%s", ENV{NM_UNMANAGED}="0"\n'
)
NM_UNMANAGED_MAC = (
    'SUBSYSTEM=="net", ACTION=="add|change|move", ATTR{address}=="%s", ENV{NM_UNMANAGED}="1"\n'
)
NM_MANAGED_DRIVER = (
    'SUBSYSTEM=="net", ACTION=="add|change|move", ENV{ID_NET_DRIVER}=="%s", ENV{NM_UNMANAGED}="0"\n'
)
NM_UNMANAGED_DRIVER = (
    'SUBSYSTEM=="net", ACTION=="add|change|move", ENV{ID_NET_DRIVER}=="%s", ENV{NM_UNMANAGED}="1"\n'
)
    ...
    def normalize_yaml_tree(self, data, full_key=""):
        ...
        if isinstance(data, list):
            scalars_only = not any(
                list(map(lambda elem: (isinstance(elem, dict) or isinstance(elem, list)), data))
            )
            ...
            if (
                "renderer" in keys
                and len(full_key.split(":")) > 1
                and data["renderer"] in ["networkd", "NetworkManager"]
            ):
                del data["renderer"]
...
class TestBase(unittest.TestCase):
    ...
    def validate_generated_yaml(self, yaml_input):
        ...
        for input in yaml_input:
            ...
            with open(input, "r") as orig:
                ...
                if (
                    input_yaml is None
                    or input_yaml == {"network": {}}
                    or input_yaml == {"network": {"version": 2}}
                ):
            ...
            if yaml_files_differ:  # pragma: no cover (only execited in error case)
                ...
                for line in difflib.unified_diff(
                    input_lines, output_lines, fromfile, tofile="generated", lineterm=""
                ):
    ...
    def generate(
        ...
        extra_args=[],
        confs=None,
        skip_generated_yaml_validation=False,
    ):
    ...
    def assert_networkd(self, file_contents_map):
        ...
        self.assertEqual(
            set(os.listdir(networkd_dir)), {"10-netplan-" + f for f in file_contents_map}
        )
    ...
    def assert_networkd_udev(self, file_contents_map):
        ...
        if not file_contents_map:
            ...
            self.assertTrue(
                (not os.path.exists(udev_dir)) or (os.listdir(udev_dir) == ["90-netplan.rules"])
            )
            return

        self.assertEqual(
            set(os.listdir(udev_dir)) - set(["90-netplan.rules"]),
    ...
    def assert_nm(self, connections_map=None, conf=None):
        ...
        conf_path = os.path.join(
            self.workdir.name, "run", "NetworkManager", "conf.d", "netplan.conf"
        )
        if conf:
    ...
    def assert_sriov(self, file_contents_map):
        ...
        self.assertEqual(
            set(os.path.basename(file) for file in sriov_systemd_dir),
