...
class TestConfigArgs(TestBase):
  ...
  ethernets:
    eth0:
      dhcp4: true""",
            expect_fail=True,
            extra_args=["/non/existing/config"],
        )
  ...
  ethernets:
    eth0:
      dhcp4: true"""
        ...
        p = subprocess.Popen(
            [exe_generate, "--root-dir", self.workdir.name, "--help"],
    ...
    def test_unknown_cli_args(self):
        p = subprocess.Popen(
            [exe_generate, "--foo"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
        )
        (out, err) = p.communicate()
  ...
  ethernets:
    eth0:
      dhcp4: true"""
        ...
        self.assertTrue(
            os.path.islink(
                os.path.join(outdir, "multi-user.target.wants", "systemd-networkd.service")
            )
        )
        self.assertTrue(
            os.path.islink(
        ...
        out = subprocess.check_output(
            [generator, "--root-dir", self.workdir.name, outdir, outdir, outdir],
    ...
    def test_systemd_generator_badcall(self):
        ...
        try:
            subprocess.check_output(
                [generator, "--root-dir", self.workdir.name], stderr=subprocess.STDOUT
            )
            self.fail(
                "direct systemd generator call is expected to fail, but succeeded."
            )  # pragma: nocover
