...
""",
        ...
        self.assert_networkd(
            {
                "ovs0.network": ND_EMPTY % ("ovs0", "ipv6"),
                "eth0.network": (ND_DHCP6 % "eth0").replace(
                    "LinkLocalAddressing=ipv6", "LinkLocalAddressing=no\nBridge=ovs0"
                ),
                "eth1.network": (ND_DHCP4 % "eth1").replace(
                    "LinkLocalAddressing=ipv6", "LinkLocalAddressing=no\nBridge=ovs0"
                ),
            }
        )
  ...
  ethernets:
    eth0:
      openvswitch:
        ...
        self.assertIn(
            "eth0: Interface needs to be assigned to an OVS bridge/bond to carry external-ids/other-config",
            err,
        )
  ...
  bridges:
    ovs0:
      openvswitch: {}""",
        ...
        )  # OpenFlow16 won't be re-generated
...
"""
        ...
        self.assert_ovs(
            {
                "bond0.service": OVS_VIRTUAL
                % {
                    "iface": "bond0",
                    "extra": """Requires=netplan-ovs-br0.service
...
""",
        ...
        self.assertIn(
            "Key 'controller.connection-mode' is only valid for interface type 'openvswitch bridge'",
            err,
        )
        self.assert_ovs({})
        self.assert_networkd({})
...
""",
        ...
        self.assertIn(
            "Key 'controller.addresses' is only valid for interface type 'openvswitch bridge'", err
        )
        self.assert_ovs({})
        self.assert_networkd({})
...
""",
        ...
        self.assertIn(
            "ERROR: openvswitch bridge controller target 'ssl:10.10.10.1' needs SSL configuration, but global \
'openvswitch.ssl' settings are not set",
            err,
        )
        self.assert_ovs({"cleanup.service": OVS_CLEANUP % {"iface": "cleanup"}})
        self.assert_networkd({})
...
""",
                ...
                "patchx.service": OVS_VIRTUAL
                % {
                    "iface": "patchx",
                    "extra": """Requires=netplan-ovs-br1.service
...
""",
                ...
                "patchy.service": OVS_VIRTUAL
                % {
                    "iface": "patchy",
                    "extra": """Requires=netplan-ovs-bond0.service
...
""",
        ...
        self.assert_networkd(
            {
                "br0.network": ND_WITHIP % ("br0", "192.168.1.1/24"),
...
"""
        ...
        self.assert_networkd(
            {
                "non-ovs-bond.network": ND_EMPTY % ("non-ovs-bond", "no") + "Bridge=ovs-br\n",
                "eth1.network": (ND_EMPTY % ("eth1", "no")).replace(
                    "ConfigureWithoutCarrier=yes", "Bond=non-ovs-bond"
                ),
                "eth0.network": (ND_EMPTY % ("eth0", "no")).replace(
                    "ConfigureWithoutCarrier=yes", "Bond=non-ovs-bond"
                ),
