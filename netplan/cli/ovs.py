...
def _del_col(type, iface, column, value):
    ...
    if default is None:
        ...
        subprocess.check_call(
            [OPENVSWITCH_OVS_VSCTL, "set", type, iface, "%s=%s" % (column, default)]
        )
...
def _del_dict(type, iface, column, key, value):
    ...
    # removes the exact value only if it was set by netplan
    subprocess.check_call(
        [OPENVSWITCH_OVS_VSCTL, "remove", type, iface, column, key, _escape_colon(value)]
    )
...
def apply_ovs_cleanup(
    config_manager, ovs_old, ovs_current
        ...
        for t in (("Port", "del-port"), ("Bridge", "del-br"), ("Interface", "del-br")):
            ...
            for line in out.splitlines():
                if "netplan=true" in line:
                    ...
                    if (
                        t[0] == "Interface"
                        and subprocess.run([OPENVSWITCH_OVS_VSCTL, "br-exists", iface]).returncode
                        > 0
                    ):
                        subprocess.check_call(
                            [OPENVSWITCH_OVS_VSCTL, "--if-exists", "del-bond-iface", iface]
                        )
                    else:
