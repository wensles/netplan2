...
class PCIDevice(object):
    ...
    def vf_addrs(self) -> list:
        ...
        while True:
            try:
                vf_addrs.append(os.path.basename(os.readlink(self.subpath("virtfn{}".format(i)))))
                ...
                for interface in interfaces:
                    ...
                    if pf_link in pfs:
                        raise ConfigurationError(
                            "matched more than one interface for a PF device: %s" % pf_link
                        )
...
def get_vf_count_and_functions(interfaces, np_state, vf_counts, vfs, pfs):
    """
    ...
    for nid, netdef in np_state.ethernets.items():
        if netdef.sriov_link and _get_target_interface(
            interfaces, np_state, netdef.sriov_link.id, pfs
        ):
            vfs[nid] = None
...
def set_numvfs_for_pf(pf, vf_count):
    ...
    try:
        ...
        if e.errno == 16:  # device or resource busy
            logging.warning(
                "device or resource busy while setting sriov_numvfs for %s, trying workaround" % pf
            )
            try:
        ...
        if bail:
            raise RuntimeError(
                "failed setting sriov_numvfs to %s for %s: %s" % (vf_count, pf, str(e))
            )
...
def apply_vlan_filter_for_vf(pf, vf, vlan_name, vlan_id, prefix="/"):
    ...
    try:
        subprocess.check_call(
            ["ip", "link", "set", "dev", pf, "vf", vf_index, "vlan", str(vlan_id)],
...
def apply_sriov_config(config_manager, rootdir="/"):
    ...
    get_vf_count_and_functions(interfaces, np_state, vf_counts, vfs, pfs)
            ...
            for interface in interfaces:
                if netdef.match_interface(itf_name=interface):
                    if vf in vfs and vfs[vf]:
                        raise ConfigurationError(
                            "matched more than one interface for a VF device: %s" % vf
                        )
                    vfs[vf] = interface
        else:
            if vf in interfaces:
