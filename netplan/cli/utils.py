...
def is_nm_snap_enabled():
    return (
        subprocess.call(
            ["systemctl", "--quiet", "is-enabled", NM_SNAP_SERVICE_NAME], stderr=subprocess.DEVNULL
        )
        == 0
    )
...
def systemctl_is_masked(unit_pattern):
    ...
    res = subprocess.run(
        ["systemctl", "is-enabled", unit_pattern],
...
def ip_addr_flush(iface):
    ...
    subprocess.check_call(
        ["ip", "addr", "flush", iface], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )


def get_interface_driver_name(
    interface, only_down=False
...
def find_matching_iface(interfaces: list, netdef):
    ...
    matches = list(
        filter(
            lambda itf: netdef.match_interface(
                itf_name=itf,
...
class NetplanCommand(argparse.Namespace):
    def __init__(self, command_id, description, leaf=True, testing=False):
        ...
        self.parser = argparse.ArgumentParser(
            prog="%s %s" % (sys.argv[0], command_id), description=description, add_help=True
        )
        self.parser.add_argument("--debug", action="store_true", help="Enable debug messages")
        self.parser.add_argument("--breakpoint", action="store_true", help=argparse.SUPPRESS)
        if not leaf:
            self.subparsers = self.parser.add_subparsers(
                title="Available commands", metavar="", dest="subcommand"
            )
            p_help = self.subparsers.add_parser(
                "help", description="Show this help message", help="Show this help message"
            )
