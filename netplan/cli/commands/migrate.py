...
class NetplanMigrate(utils.NetplanCommand):
    ...
    def parse_mtu(self, iface, if_options, if_config):
        ...
        if "mtu" in if_options:
            ...
            if "mtu" in if_config and not if_config["mtu"] == mtu:
                logging.error(
                    "%s: tried to set MTU=%d, but already have MTU=%d", iface, mtu, if_config["mtu"]
                )
                sys.exit(2)
    ...
    def parse_hwaddress(self, iface, if_options, if_config):
        ...
        if "hwaddress" in if_options:
            if "macaddress" in if_config and not if_config["macaddress"] == if_options["hwaddress"]:
                logging.error(
                    "%s: tried to set MAC %s, but already have MAC %s",
                    iface,
                    if_options["hwaddress"],
                    if_config["macaddress"],
                )
                sys.exit(2)
    ...
    def run(self):
        ...
        if NO_YAML:  # pragma: nocover
            logging.error(
                """The `yaml` Python package couldn't be imported, and is needed for the migrate command.
        ...
        for iface, family_config in ifaces.items():
            for family, config in family_config.items():
                ...
                if config["method"] == "loopback":
                    ...
                    c = (
                        netplan_config.setdefault("network", {})
                        .setdefault("ethernets", {})
                        .setdefault(iface, {})
                    )
                    ...
                    if config["options"]:
                        logging.error(
                            "%s: option(s) %s are not supported for dhcp method",
                            iface,
                            ", ".join(config["options"].keys()),
                        )
                        sys.exit(2)
                    if family == "inet":
                        c["dhcp4"] = True
                    ...
                    if family == "inet":
                        ...
                        if bad_opts:
                            for unsupported in bad_opts.intersection(unsupported_opts):
                                logging.error(
                                    '%s: unsupported %s option "%s"', iface, family, unsupported
                                )
                                sys.exit(2)
                        ...
                        if "/" in config["options"]["address"]:
                            ...
                            if "netmask" not in config["options"]:
                                logging.error(
                                    "%s: address does not specify prefix length, and netmask not specified",
                                    iface,
                                )
                                sys.exit(2)
                            addr_spec = config["options"]["address"]
                            net_spec = (
                                config["options"]["address"] + "/" + config["options"]["netmask"]
                            )
                        ...
                        try:
                            ...
                            logging.error(
                                '%s: error parsing "%s" as an IPv4 address: %s', iface, addr_spec, a
                            )
                            sys.exit(2)
                        ...
                        try:
                            ...
                            logging.error(
                                '%s: error parsing "%s" as an IPv4 network: %s', iface, net_spec, a
                            )
                            sys.exit(2)
                    ...
                    # ipv6
                        ...
                        try:
                            ...
                            logging.error(
                                '%s: error parsing "%s" as an IPv6 address: %s', iface, addr_spec, a
                            )
                            sys.exit(2)
                        ...
                        try:
                            ...
                            logging.error(
                                '%s: error parsing "%s" as an IPv6 network: %s', iface, net_spec, a
                            )
                            sys.exit(2)
                        ...
                        if "accept_ra" in config["options"]:
                            if config["options"]["accept_ra"] == "0":
                                ...
                                logging.error(
                                    '%s: unexpected accept_ra value "%s"',
                                    iface,
        ...
        if netplan_config:
            ...
            if self.dry_run:
                ...
                try:
                    ...
                    logging.error(
                        "%s already exists; remove it if you want to run the migration again", dest
                    )
                    sys.exit(3)
                ...
                for line in f:
                    ...
                    if line.startswith("source-directory "):
                        ...
                        for f in os.listdir(d):
                            if valid_re.match(f):
                                lines += self._ifupdown_lines_from_file(
                                    rootdir, os.path.join(d[rootdir_len:], f)
                                )
    ...
    def parse_ifupdown(self, rootdir="/"):
        ...
        for line in lines:
            ...
            if len(fields) != exp_len + 1:
                raise ValueError(
                    "Expected %i fields for stanza type %s but got %i"
            ...
            if fields[0] in ("auto", "allow-auto", "allow-hotplug"):
                ...
                in_options = fields[1]
                in_family = fields[2]
                ifaces.setdefault(fields[1], OrderedDict())[in_family] = {
                    "method": fields[3],
                    "options": {},
                }
            else:
                raise NotImplementedError(
                    "stanza type %s is not implemented" % fields[0]
                )  # pragma nocover
