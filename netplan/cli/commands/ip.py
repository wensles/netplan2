...
class NetplanIp(utils.NetplanCommand):
    def __init__(self):
        super().__init__(
            command_id="ip", description="Retrieve IP information from the system", leaf=False
        )
    ...
    def run(self):
        ...
        # subcommand: leases
        p_ip_leases = self.subparsers.add_parser("leases", help="Display IP leases", add_help=False)
...
class NetplanIpLeases(utils.NetplanCommand):
    def __init__(self):
        super().__init__(command_id="ip leases", description="Display IP leases", leaf=True)

    def run(self):
        self.parser.add_argument(
            "interface", help="Interface for which to display IP lease settings."
        )
        self.parser.add_argument(
            "--root-dir", help="Search for configuration files in this root directory instead of /"
        )
    ...
    def command_ip_leases(self):
        ...
        def find_lease_file(mapping):
            ...
            def lease_method_nm_connection():
                ...
                try:
                    ...
                    raise Exception(
                        "Could not find a NetworkManager connection for the interface: %s" % str(e)
                    )
            ...
            try:
                ...
                path = os.path.join(
                    ...
                    lease_pattern.format(interface=self.interface, lease_id=lease_id),
                )
                ...
                print(
                    "No lease found for interface '%s': %s" % (self.interface, str(e)),
                    file=sys.stderr,
                )
                sys.exit(1)
        ...
        try:
            ...
            print(
                "No lease found for interface '%s' (not managed by Netplan)" % self.interface,
                file=sys.stderr,
            )
            sys.exit(1)
        mapping = {}
