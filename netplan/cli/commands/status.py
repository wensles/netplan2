...
class Interface:
    ...
    def __init__(
        ...
        nd_data: JSON = [],
        nm_data: JSON = [],
        resolved_data: tuple = (None, None),
        route_data: tuple = (None, None),
    ):
        ...
        if resolved_data[0]:
            ...
            for itr in resolved_data[0]:
                if int(itr[0]) == int(self.idx):
                    ipfamily = itr[1]
                    dns = itr[2]
                    self.dns_addresses.append(
                        socket.inet_ntop(ipfamily, b"".join([v.to_bytes(1, "big") for v in dns]))
                    )
            ...
            for addr in addr_info:
                ...
                if self.routes:
                    for route in self.routes:
                        if "from" in route and ipaddress.ip_address(
                            route["from"]
                        ) == ipaddress.ip_address(addr["local"]):
    ...
    def query_nm_ssid(self, con_name: str) -> str:
        ssid: str = None
        try:
            ssid = utils.nmcli_out(
                ["--get-values", "802-11-wireless.ssid", "con", "show", "id", con_name]
            )
            return ssid.strip()
        except Exception as e:
            logging.warning("Cannot query NetworkManager SSID for {}: {}".format(con_name, str(e)))
        return ssid
    ...
    def query_networkctl(self, ifname: str) -> str:
        ...
        try:
            ...
            logging.warning("Cannot query networkctl for {}: {}".format(ifname, str(e)))
        return output
    ...
    def backend(self) -> str:
        if (
            self.nd
            and "unmanaged" not in self.nd.get("SetupState", "")
            and "run/systemd/network/10-netplan-" in self.nd.get("NetworkFile", "")
        ):
            return "networkd"
        elif self.nm and "run/NetworkManager/system-connections/netplan-" in self.nm.get(
            "filename", ""
        ):
    ...
    def netdef_id(self) -> str:
        if self.backend == "networkd":
            return (
                self.nd.get("NetworkFile", "")
                .split("run/systemd/network/10-netplan-")[1]
                .split(".network")[0]
            )
        elif self.backend == "NetworkManager":
            netdef = (
                self.nm.get("filename", "")
                .split("run/NetworkManager/system-connections/netplan-")[1]
                .split(".nmconnection")[0]
            )
...
class NetplanStatus(utils.NetplanCommand):
    ...
    def run(self):
        self.parser.add_argument(
            "ifname", nargs="?", type=str, default=None, help="Show only this interface"
        )
        self.parser.add_argument(
            "-a", "--all", action="store_true", help="Show all interface data (incl. inactive)"
        )
        self.parser.add_argument(
            "-f",
            "--format",
    ...
    def query_iproute2(self) -> JSON:
        data: JSON = None
        try:
            output: str = subprocess.check_output(["ip", "-d", "-j", "addr"], text=True)
    ...
    def query_networkd(self) -> JSON:
        data: JSON = None
        try:
            output: str = subprocess.check_output(["networkctl", "--json=short"], text=True)
    ...
    def query_nm(self) -> JSON:
        data: JSON = None
        try:
            output: str = utils.nmcli_out(
                ["-t", "-f", "DEVICE,NAME,UUID,FILENAME,TYPE,AUTOCONNECT", "con", "show"]
            )
    ...
    def query_routes(self) -> tuple:
        ...
        data6 = None
        try:
            output4: str = subprocess.check_output(["ip", "-d", "-j", "route"], text=True)
            data4: JSON = self.process_generic(output4)
            output6: str = subprocess.check_output(["ip", "-d", "-j", "-6", "route"], text=True)
    ...
    def pretty_print(self, data: JSON, total: int, _console_width=None) -> None:
        ...
        console = Console(
            highlighter=NetplanHighlighter(), theme=theme, width=_console_width, emoji=False
        )
        ...
        # Global state
        pprint(
            ("{title:>" + pad + "} {value}").format(
                title="Online state:",
                value="[online]online[/online]"
                if global_state.get("online", False)
                else "[offline]offline[/offline]",
            )
        )
        ...
        if dns_addr:
            for i, val in enumerate(dns_addr):
                pprint(
                    ("{title:>" + pad + "} {value}[muted]{mode}[/muted]").format(
        ...
        for ifname, data in interfaces:
            ...
            pprint(
                "[{col}]●[/{col}] {idx:>2}: {name} {type} [{col}]{state}[/{col}] ({backend}{netdef})".format(
                    col=scolor,
                    idx=data.get("index", "?"),
                    name=ifname,
                    type=full_type,
                    ...
                    netdef=": [highlight]{}[/highlight]".format(data.get("id"))
                    if data.get("id")
                    else "",
                )
            )
            ...
            if data.get("macaddress"):
                pprint(
                    ("{title:>" + pad + "} {mac}[muted]{vendor}[/muted]").format(
                        title="MAC Address:",
            ...
            if lst:
                for i, obj in enumerate(lst):
                    ...
                    pprint(
                        (
                            "{title:>" + pad + "} {start}{ip}/{prefix}{end}[muted]{extra}[/muted]"
                        ).format(
                ...
                for i, obj in enumerate(lst):
                    ...
                    pprint(
                        (
                            "{title:>"
                            + pad
                            + "} {start}{to}{via}{src}{metric}{end}[muted]{extra}[/muted]"
                        ).format(
    ...
    def command(self):
        ...
        if not utils.systemctl_is_active("systemd-networkd.service"):
            if utils.systemctl_is_masked("systemd-networkd.service"):
                logging.error(
                    "'netplan status' depends on networkd, "
        ...
        interfaces = [
            Interface(itf, networkd, nmcli, (dns_addresses, dns_search), (route4, route6))
            for itf in iproute2
        ]
        ...
        # Global state
        data = {
            "netplan-global-state": {"online": online_state, "nameservers": self.resolvconf_json()}
        }
        # Per interface
